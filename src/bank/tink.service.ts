...
export class TinkService {
  ...
  async getTransactions(accessToken: string): Promise<TransactionRow[]> {
    try {
      ...
      this.logger.log(`Successfully fetched ${data.transactions?.length || 0} transactions`);
      return (
  ...
  async exchangeToken(code: string): Promise<TokenData> {
    try {
      ...
      if (!clientSecret || !clientId) {
        this.logger.error('TINK_CLIENT_SECRET or TINK_CLIENT_ID is not configured');
        throw new BadRequestException(
          'TINK_CLIENT_SECRET or TINK_CLIENT_ID is not configured. Please set it in .env file.',
        );
      }
      ...
      throw new BadRequestException('Failed to exchange token with Tink');
    }
  }
}
