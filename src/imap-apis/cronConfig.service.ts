...
export class CronConfigService {
  ...
  async setupCronForUser(userId: string) {
    ...
    if (!allowedIntervals.includes(selectedInterval)) {
      console.log(`Interval ${selectedInterval} not allowed in subscription plan`);
      return;
    }

    await this.createCronForUser(userId, imapConfig.realtimeImapChecking.cronTime);
  }
  ...
  async runSyncJob(userId: string) {
    try {
      ...
      await this.imapApisService.readEmailTransactionsSince(userId, lastSyncDate as Date);
